...
@Component({
  selector: 'app-med-staff',
  templateUrl: './med-staff.component.html',
  styleUrls: ['./med-staff.component.css']
})
export class MedStaffComponent implements OnInit {
  ...
  ngOnInit(): void {
    localStorage.setItem("title","Staff - Medical")           
    if (String(localStorage.getItem("title")) != "Staff - Medical") {
  ...
  getMedicalStaff() {
    this.settingsService.allMedicalStaff(this.hospitalID).subscribe(data => {
      this.staffs = data;
    })
  }
  ...
  get specialisation() {
    return this.updateForm.get('specialisation');
  }

  get admin() {
    return this.updateForm.get('admin');
  }
  ...
  update(id: string) {
    if (localStorage.getItem("superadmin")) {
      ...
      if (this.selectedDays == null) {
        ...
        this.staff.type = "medical";
        this.staff.specialisation = this.specialisation?.value;
        this.staff.onDuty = this.selectedDays;
        this.staff.admin = this.admin?.value;
  ...
  get Aspecialisation() {
    if (this.Arole?.value == "doctor") {
      this.addForm.get('specialisation')?.addValidators(Validators.required);
    } else {
      this.addForm.get('specialisation')?.reset();
    }
  ...
  addStaff() {
    if (localStorage.getItem("superadmin")) {
      ...
      if (this.count == 0) {
        ...
        this.staff.type = "medical";
        this.staff.specialisation = this.Aspecialisation?.value;
        this.staff.onDuty = this.selectedDays;
